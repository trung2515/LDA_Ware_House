...
export class ShiftComponent implements OnInit {
  ...
  initFilter() {
    this.adminService.getListTypeBill().subscribe(d => {
      this.listType = d
        .filter((t: TypeBillInfo) => {
          return t.codeTypeBill == 'NDM' || t.codeTypeBill == 'NDL';
        })
        .map((t: any) => new OptionModel(t));
    });
    ...
    forkJoin([
      ...
      ([
        ...
        this.listProductRange = product_types.map(
          (p: any) => new OptionModel(p)
        );
  ...
  showDetailShift(data: any) {
    ...
    if (data != null) {
      ...
      if (data.appointmentData.id) {
        ...
        this.shiftDetail = this.shiftDetails.filter(
          s => s.idMaster == data.appointmentData.id
        );
  ...
  getData() {
    ...
    this.adminService.getListShiftDetail(fromDate, toDate).subscribe(data => {
      console.log(data);
      this.shiftDetails = [];
    ...
    //   this.listParcel = data.map(d => new OptionModel2(d));
    // });
  }
  ...
  onAppointmentFormOpening(data: any) {
    ...
    data.cancel = true;
    if (
      this.appointments.filter(a => a.id == data.appointmentData.id).length > 0
    ) {
